...
N_core = 10
compression = 'lzf'
N_sources = hp.nside2npix(total_nside)
N_block = hp.nside2npix(block_nside)
N_chunk = int(N_sources/N_block)
N_maxobs = 256

if compression == 'gzip':
    filename = f'gaiaedr3_times_healpix_{total_nside}_{block_nside}_gzip'
else:
    filename = f'gaiaedr3_times_healpix_{total_nside}_{block_nside}'
print('Using',N_core,'cores to process',N_block,'blocks.')
print(filename)
...
def mp_worker(args):
    ...
    # Load in data
    source_id = np.arange(block_idx*N_chunk,(block_idx+1)*N_chunk).astype(np.int)
    source_xyz = np.stack(hp.pix2vec(nside = total_nside, ipix = source_id, nest = True)).T
    ...
    with h5py.File(f'./store/{filename}_{block_idx}.h5', 'w') as f:
...
if True:
    ...
    with h5py.File(f'{filename}.h5', 'w') as f:
        ...
        # Create datasets
        if compression == 'gzip':
            f.create_dataset('source_id', compression="gzip", compression_opts=9, chunks = (N_chunk,), shape = (N_sources,), dtype = np.uint64, fletcher32 = False, shuffle = True, scaleoffset=0)
            f.create_dataset('fov_1_n', compression="gzip", compression_opts=9, chunks = (N_chunk,), shape = (N_sources,), dtype = np.uint8, fletcher32 = False, shuffle = True, scaleoffset=0)
            f.create_dataset('fov_2_n', compression="gzip", compression_opts=9, chunks = (N_chunk,), shape = (N_sources,), dtype = np.uint8, fletcher32 = False, shuffle = True, scaleoffset=0)
            f.create_dataset('fov_1_times', compression="gzip", compression_opts=9, chunks = (N_chunk, N_maxobs, ), shape = (N_sources, N_maxobs, ), dtype = np.uint32, fletcher32 = False, shuffle = True, scaleoffset=0)
            f.create_dataset('fov_2_times', compression="gzip", compression_opts=9, chunks = (N_chunk, N_maxobs, ), shape = (N_sources, N_maxobs, ), dtype = np.uint32, fletcher32 = False, shuffle = True, scaleoffset=0)
        else:
...
with h5py.File(f'{filename}.h5', 'a') as f:
    ...
    for result in tqdm.tqdm(pool.imap(mp_worker, range(N_block)),total=N_block):
        ...
        with h5py.File(f'./store/{filename}_{block_idx}.h5', 'r') as g:
        ...
        os.remove(f'./store/{filename}_{block_idx}.h5')
