...
The parameter :math:`\mathcal{R}` is the current Learning Rate - which we modify from the usual constant of the ADAM optimiser. The learning rate is the product of the LearningRate parameter and the Harness parameter.
...
The learning rate is nominally set to the step size of the optimizer, however, it can change on two conditions:

1. If, over the course of an epoch, the net change in the function is positive (I.e. minimization did not occur), the learning rate is halved.
2. If, over the course of an epoch, the net change in the function is positive, the learning rate increases by 3%. This cannot increase the learning rate above the original stepsize. 
